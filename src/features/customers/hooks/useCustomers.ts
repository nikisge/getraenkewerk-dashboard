...
export type SortField = 'firma' | 'plz' | 'ort' | 'revenue_365d';
...
export function useCustomers(
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<CustomersResult> => {
      const { searchTerm, repId, statusActive, abcClass, incompleteOnly, plzPrefix, sortBy, sortDir } = filters;
      ...
      let query = supabase
        .from("dim_customers")
        .select("kunden_nummer, firma, email, ort, plz, strasse, telefon, mobil, u_key, rep_id, purchase_interval, season_start, season_end, status_active, abc_class, revenue_365d, latitude, longitude, opening_hours_mon, opening_hours_tue, opening_hours_wed, opening_hours_thu, opening_hours_fri, opening_hours_sat, opening_hours_sun, opening_hours_notes, contact", { count: "exact" })
        .range(from, to);

      // Apply dynamic sorting
      const orderColumn = sortBy || 'firma';
      const ascending = sortDir !== 'desc';
      // For revenue, nulls should be last when sorting desc (highest first)
      if (orderColumn === 'revenue_365d') {
      ...
      // PLZ prefix filter (e.g., "20" matches all 20xxx postal codes)
      if (plzPrefix?.trim()) {
        query = query.ilike("plz", `${plzPrefix.trim()}%`);
      }
